...
def gather_opennmt_data(parent_dir, outdir):
                ...
                if os.path.isdir(paper_path):
                    ...
                    for eqn_path in equation_dirs:
                        ...
                        if os.path.exists(eqn_tex_file) and os.path.exists(eqn_pdf_file):
                            ...
                            counter+=1
    print("Processed {0} equation instances!".format(counter))
    ...
    # Output:
    # Source: Images
    save_text(os.path.join(outdir, 'src-train.txt'), tr_imgs)
    save_text(os.path.join(outdir, 'src-val.txt'), dev_imgs)
    save_text(os.path.join(outdir, 'src-test.txt'), test_imgs)
    # Target: Tex
    save_text(os.path.join(outdir, 'tgt-train.txt'), tr_eqns)
    save_text(os.path.join(outdir, 'tgt-val.txt'), dev_eqns)
    save_text(os.path.join(outdir, 'tgt-test.txt'), test_eqns)
