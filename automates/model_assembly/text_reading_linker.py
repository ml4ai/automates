...
class TextReadingLinker:
                    ...
                    TypedMetadata.from_data(
                        {
                            ...
                            "provenance": provenance,
                            "equation_extraction": equation_parameter[
                                "equation_extraction"
                            ],
                            "variable_identifier": grounding["gvar"],
    ...
    def build_equation_groundings(self, gvar: GVarNode, L):
        ...
        selected_g_eq_var = max(
            equation_vars, key=lambda eq: L.edges[eq, gvar]["weight"]
        )
        ...
        return [
            ...
            for selected_eqn_var in selected_g_eq_var.equation_nodes
        ]

    def get_links_from_graph(self, L):
        grfn_var_to_groundings = {}
        for gcode_var in [n for n in L.nodes if isinstance(n, GCodeVarNode)]:
            gcode_var_name = gcode_var.content
            ...
            for gcomm, gvars in gcomm_nodes_to_gvars.items():
                for gvar in gvars:
                    score = min(
                        L.edges[gcomm, gcode_var]["weight"],
                        L.edges[gvar, gcomm]["weight"],
                    )
                    if (
    ...
    def perform_tr_grfn_linking(self, grfn: GroundedFunctionNetwork, tr_sources: dict):
        ...
        for document_type in ["doc_file", "comm_file", "eqn_file"]:
            if document_type not in tr_sources:
                print(
                    f"Error: required TR source {document_type} not passed "
        ...
        # the hypothesis
        self.text_reading_interface.extract_mentions(
