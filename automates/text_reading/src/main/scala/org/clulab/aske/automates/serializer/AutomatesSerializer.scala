...
import org.clulab.aske.automates.attachments.{DiscontinuousCharOffsetAttachment, MentionLocationAttachment, ParamSetAttachment, ParamSettingIntAttachment, UnitAttachment, ContextAttachment}
...
object AutomatesJSONSerializer {
  ...
  def toAttachment(json: ujson.Value): Attachment = {
    ...
    val toReturn = attType match {
      ...
      case "ContextAtt" => new ContextAttachment(attType, json("contexts").arr, foundBy)
      case _ => ???
    }
    toReturn
  }
