...
//  val inputDir: String = "/Users/alexeeva/Desktop/automates-related/arxiv/testSubscriptDataScript"
  ...
  val existingFiles = FileUtils.findFiles(outputDir, "tsv")
  ...
  //  pw.write("MAsha")
  files.foreach { file =>
    val newFileName = new File(outputDir + file.getName.replace(".tex", "") + "subscripts_data_sample.tsv")
    if (!existingFiles.contains(newFileName)) {
    try {
