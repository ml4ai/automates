...
class TestPairwiseW2VAligner extends FlatSpec with Matchers {
  ...
  it should "extract relevant portions of mentions" in {
    ...
    val variable = new TextBoundMention("Identifier", Interval(0,1), 0, doc1, true, "<MANUAL>")
    ...
    val rm = new RelationMention("Description", Map("variable" -> Seq(variable), "description" -> Seq(description), "conditional" -> Seq(condition)),
      0, doc1, true, "<MANUAL>")
