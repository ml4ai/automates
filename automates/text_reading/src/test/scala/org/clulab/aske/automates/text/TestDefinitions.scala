...
// Tests from paper: 2003-A_double_epidemic_model_for_the_SARS_propagation
  ...
  passingTest should s"find definitions from t2gToy: ${t2gToy}" taggedAs(Somebody) in {
    val desired =  Seq(
  ...
  passingTest should s"find definitions from t3g: ${t3g}" taggedAs(Somebody) in {
    val desired =  Seq(
...
// Tests from unknown papers
  ...
  passingTest should s"find definitions from t1h: ${t1h}" taggedAs(Somebody) in {
    val desired =  Seq(
  ...
  failingTest should s"find definitions from t3h: ${t3h}" taggedAs(Somebody) in {
    val desired =  Seq(
      "ETc" -> Seq("crop evapotranspiration under standard conditions"), // todo: ETc should be captured as a variable. -> fixed (definitions not captured properly. needs new rules.)
...
// Tests from paper: Covid_Act_Now_Model_Reference
  ...
  failingTest should s"find definitions from t1i: ${t1i}" taggedAs(Somebody) in {
    val desired =  Seq(
      "S" -> Seq("Susceptible"),
      "E" -> Seq("exposed"), // fixme: Definitions of I1, I2, I3 are not extracted (needs to be reviewed - needs a new conj_rule?) // todo: check if I1, I2, I3 are actually variables (covid act now p.2)
      "I" -> Seq("infected"),
      "I1" -> Seq("mild"),
      "I2" -> Seq("moderate"),
      "I3" -> Seq("severe")
...
// Tests from paper: THE ASCE STANDARDIZED REFERENCE EVAPOTRANSPIRATION EQUATION
  ...
  failingTest should s"find definitions from t2l: ${t2l}" taggedAs(Somebody) in {
    val desired =  Seq(
      "ETsz" -> Seq("standardized reference crop evapotranspiration for short (ETos) or tall (ETrs) surfaces") //fixme: short is captured as the definition of ETos
    )
