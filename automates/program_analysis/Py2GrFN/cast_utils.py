...
def flatten_generator(l):
    for el in l:
        if (isinstance(el, Sequence) or isinstance(el, set)) and not isinstance(
            el, (str, bytes)
        ):
...
def generate_function_object(
    name,
    func_type,
    input_var_ids=list(),
    output_var_ids=list(),
    updated_var_ids=list(),
    lambda_str="",
):
    return {
        "function": {
...
def generate_variable_object(name, var_type):
    return {
        "name": name,
        "source_refs": [],
        "domain": {"name": "none", "type": var_type, "mutable": False},
        "domain_constraint": "",
    }
...
def create_container_object(containers, container_name, container_type):
    if not container_name in containers:
        ...
        containers[container_name]["cur_if_exp"] = 0
    return containers


def create_class_object(classes, class_id, name, body):
    classes[class_id] = defaultdict(lambda: list())
...
@singledispatch
def generate_decision_lambda(
    node: ast.For, cond_vars, conditions_updating_var, name, is_val_set_in_else
):
    return "lambda " + cond_vars[0] + ": not (" + cond_vars[0] + ")"


@generate_decision_lambda.register
def _(node: ast.While, cond_vars, conditions_updating_var, name, is_val_set_in_else):
    return "lambda " + cond_vars[0] + ": not (" + cond_vars[0] + ")"


@generate_decision_lambda.register
# NOTE cond_vars must be sorted in order of appearing conditions in source
def _(node: ast.If, cond_vars, conditions_updating_var, name, is_val_set_in_else):
    all_vars = []
...
def _(node: ast.ClassDef, module, scope, variable_table):
    return "@container::" + module + "::" + scope_to_string(scope)
...
def _(node: ast.If, module, scope, variable_table):
    return (
        "@container::" + module + "::" + scope_to_string(scope[:-1]) + "::" + scope[-1]
    )
...
def find_variable_name_at_scope_or_higher(name, scope, variable_table):
    ...
    def has_name_and_scope(id):
        return (
            id
            and id.split("::")[2] in potential_scope_strings
            and id.split("::")[3] == name
        )

    ids_with_name = [
        var_identifier + "::" + str(variable_table[var_identifier]["version"])
        for var_identifier in variable_table.keys()
        if has_name_and_scope(var_identifier)
    ]
...
def get_largest_scope_variable_identifier(id, scope, module, variable_table):
    ...
    vars_without_other_conditions = [
        v for v in vars_with_name if in_current_conditions(v)
    ]
...
def _(node: str, module, scope, variable_table):
    variable_identifier = get_largest_scope_variable_identifier(
        node, scope, module, variable_table
    )
...
def build_dict_key_list(dict_var_obj):
    key_instances = list(dict_var_obj["key_instances"].keys())
    key_instances.sort()
    collected_keys = []
    for instance in key_instances:
        keys = dict_var_obj["key_instances"][instance]
        # Three key types, add, delete, and update
        # add all the added keys at this instance
        collected_keys.extend(keys["add"])
        # remove all of the deleted keys
        collected_keys = [x for x in collected_keys if not x in keys["delete"]]
