...
class PyASTToCAST(ast.NodeVisitor):
    ...
    def visit_AugAssign(self, node:ast.AugAssign):
        ...
        if isinstance(target,ast.Attribute):
            ...
            convert = ast.Assign(
                targets=[target], 
                value=ast.BinOp(left=ast.Name(target.value,ctx=ast.Load(),
                                                col_offset=node.col_offset,
                                                end_col_offset=node.end_col_offset,
                                                lineno=node.lineno,
                                                end_lineno=node.end_lineno
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(node.func,ast.Attribute):
            return [Call(self.visit(node.func)[0], args, source_refs=ref)]
        else:
    ...
    def visit_For(self, node: ast.For):
        ...
        ref = [SourceRef(source_file_name=self.filenames[-1], col_start=node.col_offset, col_end=node.end_col_offset, row_start=node.lineno, row_end=node.end_lineno)]
        var_name = "generated_index%"

        # TODO: Mark this is a variable that was generated by this script
        ...
        if isinstance(node.iter,ast.Call):
            loop_cond = BinaryOp(
                ...
                Call(Name("len", source_refs=ref), [iterable], source_refs=ref),
                source_refs=ref
            )
            if isinstance(node.target,ast.Tuple):
                loop_assign = [Assignment(
                    ...
                    Subscript(Call(Name("list", source_refs=ref),[iterable],source_refs=ref), Name(var_name, source_refs=ref),source_refs=ref),
                    source_refs=ref
                ),
                Assignment(
                    target,
            ...
            if isinstance(node.target,ast.Tuple):
                ...
                loop_assign = [Assignment(
                    Var(Name(node.target.id, source_refs=ref), "float", source_refs=ref),
                    Subscript(Name(node.iter.attr, source_refs=ref), Name(var_name, source_refs=ref), source_refs=ref),
                    source_refs=ref
                )]
        else:
            loop_cond = BinaryOp(
        ...
        return [count_var, Loop(expr=loop_cond, body=loop_assign + body + loop_increment, source_refs=ref)]
    ...
    def visit_Subscript(self, node: ast.Subscript):
        ...
        if isinstance(slc,ast.Slice):
            ...
            if slc.upper is not None:
                ...
                if isinstance(node.value,ast.Call):
                    if isinstance(node.value.func,ast.Attribute):
                        upper = Call(Name("len", source_refs=ref), [Name(node.value.func, source_refs=ref)], source_refs=ref)
                    else:
            ...
            if isinstance(node.value,ast.Call):
                if isinstance(node.value.func,ast.Attribute):
                    temp_list = f"{node.value.func.attr}_generated%"
                else:
                    temp_list = f"{node.value.func.id}_generated%"
            elif isinstance(node.value,ast.Attribute):
                temp_list = f"{node.value.attr}_generated%"
            else: 
                temp_list = f"{value.name}_generated%"
    ...
    def visit_With(self, node: ast.With):
        ...
        for item in node.items:
            ref = [SourceRef(source_file_name=self.filenames[-1], col_start=node.col_offset, col_end=node.end_col_offset, row_start=node.lineno, row_end=node.end_lineno)]
            variables.append([Assignment(left=self.visit(item.optional_vars)[0],right=self.visit(item.context_expr)[0], source_refs=ref)])
