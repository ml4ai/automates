...
from automates.utils.astpp import parseprint
...
class PyASTToCAST(ast.NodeVisitor):
    ...
    def visit_Assign(self, node: ast.Assign):
        ...
        if len(node.targets) == 1:
            ...
            right.extend(self.visit(node.value))
        ...
        for node in init_func:
            if type(node) == ast.Assign and type(node.targets[0]) == ast.Attribute:
                ...
                if attr_node.value.id == "self":
                    ref = [self.filenames[-1], attr_node.col_offset, attr_node.end_col_offset, attr_node.lineno, attr_node.end_lineno]
                    fields.append(Var(Name(attr_node.attr, source_refs=ref), "integer", source_refs=ref))
    ...
    def visit_Compare(self, node: ast.Compare):
        ...
        # Grab the first comparison operation using reverse
        node.ops.reverse()
    ...
    def visit_For(self, node: ast.For):
        ...
        ref = [self.filenames[-1], node.col_offset, node.end_col_offset, node.lineno, node.end_lineno]
        var_name = "i_" + str(uuid.uuid4())

        # TODO: Mark this is a variable that was generated by this script
        count_var = Assignment(Var(Name(var_name, source_refs=ref), "integer", source_refs=ref), Number(0, source_refs=ref), source_refs=ref)
        ...
        if type(node.iter) == ast.Call:
            ...
            if type(node.target) == ast.Tuple:
                loop_assign = [Assignment(
                    Tuple([Var(type="integer",val=Name(node.val.name+"_",source_refs=ref),source_refs=ref) for node in target.values],source_refs=ref),
                ...
                loop_assign = [Assignment(
                    Var(Name(node.target.id,source_refs=ref), "integer", source_refs=ref),
                    Subscript(Call(Name("list",source_refs=ref),[iterable],source_refs=ref), Name(var_name, source_refs=ref),source_refs=ref),
                    source_refs=ref
                )]
        else:
        ...
        loop_increment = [Assignment(
            Var(Name(var_name, source_refs=ref), "integer", source_refs=ref),
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        if len(node.args.args) > 0:
            args = [Var(Name(arg.arg,source_refs=[self.filenames[-1], arg.col_offset, arg.end_col_offset, arg.lineno, arg.end_lineno]), 
                    "integer", 
                    source_refs=[self.filenames[-1], arg.col_offset, arg.end_col_offset, arg.lineno, arg.end_lineno]) 
    ...
    def visit_ListComp(self, node: ast.ListComp):
        ...
        if type(first_gen.iter) == ast.Subscript:
            if type(first_gen.target) == ast.Tuple:
                ...
                outer_loop = ast.For(target=ast.Name(id=first_gen.target.id,ctx=ast.Store()),iter=first_gen.iter.value,body=[node.elt],orelse=[],col_offset=ref[1],end_col_offset=ref[2],lineno=ref[3],end_lineno=ref[4])
            ...
            if type(first_gen.target) == ast.Tuple:
                ...
                outer_loop = ast.For(target=ast.Name(id=first_gen.target.id,ctx=ast.Store()),iter=first_gen.iter.func,body=[node.elt],orelse=[],col_offset=ref[1],end_col_offset=ref[2],lineno=ref[3],end_lineno=ref[4])
        else:
            if type(first_gen.target) == ast.Tuple:
                ...
                outer_loop = ast.For(target=ast.Name(id=first_gen.target.id,ctx=ast.Store()),iter=first_gen.iter,body=[node.elt],orelse=[],col_offset=ref[1],end_col_offset=ref[2],lineno=ref[3],end_lineno=ref[4])
                ...
                if name != None: 
                    ...
                    for func in names:
                        for n in contents.body:
                            if type(n) == ast.FunctionDef and n.name == func.name: 
                                funcs.append(n)
                                break
    ...
    def visit_Name(self, node: ast.Name):
        ...
        if type(node.ctx) == ast.Store:
            if node.id in self.aliases: 
                return [Var(Name(self.aliases[node.id],source_refs=ref), "integer", source_refs=ref)]

            # TODO: Typing so it's not hardcoded to integers
            return [Var(Name(node.id,source_refs=ref), "integer", source_refs=ref)]
    ...
    def visit_Subscript(self, node: ast.Subscript):
        ...
        if type(slc) == ast.Slice:
            ...
            temp_list = value.name + "_" + str(uuid.uuid4())
            new_list = Assignment(Var(Name(temp_list, source_refs=ref), "integer", source_refs=ref),List([], source_refs=ref), source_refs=ref)

            loop_var = Assignment(Var(Name(temp_var, source_refs=ref), "integer", source_refs=ref), lower, source_refs=ref)
            ...
            loop_increment = [Assignment(
                Var(Name(temp_var, source_refs=ref), "integer", source_refs=ref),
            ...
            temp_count = 1
            for dim in dims:
                temp_list_name = lists[0] + "_" + str(temp_count)
                temp_count += 1

                list_var = Assignment(Var(Name(temp_list_name, source_refs=ref), source_refs=ref), List([], source_refs=ref), source_refs=ref)
                loop_var = Assignment(Var(Name(temp_var, source_refs=ref), "integer", source_refs=ref), Number(0, source_refs=ref),source_refs=ref)

                if type(dim) == ast.Slice:
    ...
    def visit_With(self, node: ast.With):
        ...
        This visitor unrolls the With construct and generates the appropriate cast for the
        underlying operations

        Args:
            [AstNode]: A list of CAST nodes, representing whatever operations were happening before

        """

        variables = [] 
        for item in node.items:
            ref = [self.filenames[-1], item.col_offset, item.end_col_offset, item.lineno, item.end_lineno]
