...
class PyASTToCAST(ast.NodeVisitor):
    ...
    def visit_For(self, node: ast.For):
        ...
        elif type(node.iter) == ast.Attribute:
            loop_cond = BinaryOp(
                BinaryOperator.LT,
                Name(var_name,source_refs=ref),
                Call(Name("len",source_refs=ref), [iterable], source_refs=ref),
    ...
    def visit_Subscript(self, node: ast.Subscript):
        ...
        if type(slc) == ast.Slice:
            ...
            if type(node.value) == ast.Call:
                if type(node.value.func) == ast.Attribute:
                    temp_list = node.value.func.attr + "_generated%"
                else:
                    temp_list = node.value.func.id + "_generated%"
            elif type(node.value) == ast.Attribute:
                temp_list = node.value.attr + "_generated%"
            else: 
                temp_list = value.name + "_generated%"
            ...
            for dim in dims:
                ...
                list_var = Assignment(Var(Name(temp_list_name, source_refs=ref), "integer", source_refs=ref), List([], source_refs=ref), source_refs=ref)
