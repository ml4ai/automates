...
class GrFNIntersectionGraph(nx.DiGraph):
    ...
    def from_GrFN_comparison(
        ...
        for grfn_name, grfn in named_grfns:
            ...
            # a list of node uids
            all_shared_io_paths = [[node.uid for node in shared_io_path]
        ...
        apn_member_sets = determine_abstract_path_nodes(
            per_grfn_input_node_uids, per_grfn_output_node_uids,
            per_grfn_uid_path_lists)
...
def determine_abstract_path_nodes(
    ...
    for input_uid_list, output_uid_list, io_paths in zip(
            shared_input_uid_lists, shared_outputs_uid_lists,
            input_output_path_lists):
        all_intersection_nodes = list(
            set([node for path in io_paths for node in path]))
