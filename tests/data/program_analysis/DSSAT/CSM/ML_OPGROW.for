C======================================================================
C  MZ_OPGROW, Subroutine, G. Hoogenboom, J.W. Jones
C----------------------------------------------------------------------
...
C  Calls:     None
!======================================================================
      SUBROUTINE ML_OPGROW(CONTROL, ISWITCH, 
...
!----------------------------------------------------------------------
      ...
      REAL MLAG1, TLAG1, MPLAG, TPLAG, MPLA, TPLA, PLA, MLFWT, TLFWT
      REAL MSTMWT, TSTMWT
      REAL WTLO, WTSO, WTCO
      REAL RLV(NL)
...
!**********************************************************************
     ...
     &        IOSTAT = ERRNUM)
          ...
          IF (FMOPT == 'A' .OR. FMOPT == ' ') THEN   ! VSH
          WRITE (NOUTDG, '("!",327(" ") ,"Root Dens. (cm/cm3)",
     &      " by soil depth (cm):",
     &    /,"!",T325,10A8)') (SoilProp%LayerText(L), L=1,N_LYR)
  ...
  201     FORMAT('@YEAR DOY   DAS   DAP',
     ...
     &   '     SLAD   CHTD   CWID   RDPD ',
     &   ' MLAG1  TLAG1  MPLAG  TPLAG   MPLA   TPLA    PLA',
     &   '  MLFWT  TLFWT MSTMWT TSTMWT  AMLWT  ATLWT') 
...
!           Compute average stress factors since last printout
     ...
     &        MLAG1, TLAG1, MPLAG, TPLAG, MPLA*0.0001*PLTPOP, 
     &        TPLA*.0001*PLTPOP, 
     &        PLA*0.0001*PLTPOP, MLFWT, TLFWT,
     &        MSTMWT, TSTMWT, MLFWT*PLTPOP*10., TLFWT*PLTPOP*10.
 400        FORMAT (1X,I4,1X,I3.3,2(1X,I5),1X,F6.1,1X,I6,1X,F6.2,
     &        7(1X,I6),1X,F6.1,1X,F6.3,2(1X,I6),7(1X,F6.3),3(1X,F6.2),
     &        2(1X,I6),1X,F8.1,2(1X,F6.2),1X,F6.2,13(1X,F6.2))
 ...
 404          FORMAT (3(1X,I6), 2I8, F6.2)
      ...
      IF (FMOPT == 'C') THEN    
         CALL CsvOut_MLCER(EXPNAME,CONTROL%RUN,CONTROL%TRTNUM, 
     ...
     &MLAG1, TLAG1, MPLAG, TPLAG, MPLA, TPLA, PLA, MLFWT, TLFWT,
     &MSTMWT, TSTMWT,   
     &N_LYR, RLV, WTCO, WTLO, WTSO, CUMSENSURF, 
     &CUMSENSOIL, DTT, vCsvlineMLCER, vpCsvlineMLCER, vlngthMLCER)
    
         CALL LinklstMLCER(vCsvlineMLCER)
      END IF
