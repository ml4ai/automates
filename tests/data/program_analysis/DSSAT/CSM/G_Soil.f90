SUBROUTINE SOILFILEEDIT(FILEI2,NL,TKL,SANDX, CLAYX, BD, SOC, SON, SNH4X, SNO3X, SUREA, PLOWPAN)
    ...
    WRITE(fmt3000,'(A,I2,A)') '(A8,"="',NL-1,'(F12.3,","),F12.3)'
    WRITE(IUNIT, 4000) 'NL', NL, '    ! Number of soil layers (maximum is 10) (-)'
    WRITE(IUNIT, fmt3000) 'TKL', (TKL(I), I = 1, NL)
    WRITE(IUNIT, fmt3000) 'CLAYX', (CLAYX(I), I=1, NL)
    WRITE(IUNIT, fmt3000) 'SANDX', (SANDX(I), I=1, NL)
    WRITE(IUNIT, fmt3000) 'BD', (BD(I), I=1, NL)
    WRITE(IUNIT, '(A)') "*--------------------------------------------------------------------*"
    WRITE(IUNIT, '(A)') "* 1. SOIL CHEMICAL INFORMATION (in kg C or N/ha)                                  *"
    WRITE(IUNIT, '(A)') "*--------------------------------------------------------------------*"
    WRITE(IUNIT, fmt3000) 'SOC', (SOC(I), I = 1, NL)
    WRITE(IUNIT, fmt3000) 'SON', (SON(I), I = 1, NL)
    WRITE(IUNIT, fmt3000) 'SNH4X', (SNH4X(I), I = 1, NL)
    WRITE(IUNIT, fmt3000) 'SNO3X', (SNO3X(I), I = 1, NL)
    IF(SUM(SUREA).GT.0.0) THEN
        WRITE(IUNIT, fmt3000) 'SUREA', (SUREA(I), I = 1, NL)
    END IF
