...
**/
 /** Without global thresholds**/
double saturate(double input, double lower_threshold, double upper_threshold)
{
  double output;
  if (input > upper_threshold)
    output = upper_threshold;
...
int main(int argc, char **argv)
{
    ...
    {
      //plant_command = saturate(PI_calc(desired_output, sensed_output, Kp_M, Ki_M, time_step));

      plant_command = saturate(PI_calc(desired_output, sensed_output, Kp_M, Ki_M, time_step), lower_threshold, upper_threshold);
