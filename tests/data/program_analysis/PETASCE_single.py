...
def petasce(
    ...
    udelta[0] = (
        2503.0 * math.exp(((17.27 * tavg[0]) / (tavg[0] + 237.3)))
    ) / ((tavg[0] + 237.3) ** 2.0)
    ...
    ws[0] = math.acos(
        -(
            (
                (1.0 * math.tan(((xlat[0] * pie[0]) / 180.0)))
                * math.tan(ldelta[0])
            )
        )
    )
    ra1[0] = (ws[0] * math.sin(((xlat[0] * pie[0]) / 180.0))) * math.sin(
        ldelta[0]
    )
    ra2[0] = (
        math.cos(((xlat[0] * pie[0]) / 180.0)) * math.cos(ldelta[0])
    ) * math.sin(ws[0])
    ...
    rnl[0] = ((4.901e-9 * fcd[0]) * (0.34 - (0.14 * math.sqrt(ea[0])))) * tk4[
        0
    ]
    ...
    if xhlai[0] <= 0.0:
        ...
        kcb[0] = max(
            ...
            (
                ...
                + (
                    (kcbmax[0] - kcbmin[0])
                    * (1.0 - math.exp(-(((1.0 * skc[0]) * xhlai[0]))))
                )
            ),
        )
        ...
        if meevp[0] == "G":
            kcmax[0] = max(
                (
                    1.2
                    + (
                        ((0.04 * (wnd[0] - 2.0)) - (0.004 * (rhmin[0] - 45.0)))
                        * ((cht[0] / 3.0) ** 0.3)
                    )
                ),
                (kcb[0] + 0.05),
    ...
    if kcb[0] <= kcbmin[0]:
        ...
        fc[0] = ((kcb[0] - kcbmin[0]) / (kcmax[0] - kcbmin[0])) ** (
            1.0 + (0.5 * canht[0])
        )
...
def refet(ud, rn, g, pcon, cn, tavg, w2m, es, ea, cd):
    ud: List[float]
    rn: List[float]
    g: List[float]
    pcon: List[float]
    ...
    ea: List[float]
    cd: List[float]
    rval: List[float] = [None]
    rval[0] = ((0.408 * ud[0]) * (rn[0] - g[0])) + (
        ((pcon[0] * (cn[0] / (tavg[0] + 273.0))) * w2m[0]) * (es[0] - ea[0])
    )
...
def ev_transp(ud, rn, g, pcon, cn, tavg, w2m, es, ea, cd, kc):
    ...
    cd: List[float]
    kc: List[float]
    evtransp: List[float] = [None]
    evtransp[0] = kc[0] * refet(ud, rn, g, pcon, cn, tavg, w2m, es, ea, cd)
