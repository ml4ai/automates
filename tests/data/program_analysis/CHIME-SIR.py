...
class Sir:
    def __init__(self, p: Parameters):
        ...
        self.rates = {key: d.rate for key, d in p.dispositions.items()}

        self.days = {key: d.days for key, d in p.dispositions.items()}
        ...
        infected = 1.0 / p.market_share / p.hospitalized.rate
        ...
        if p.date_first_hospitalized is None and p.doubling_time is not None:
            ...
            if p.mitigation_date is None:
                ...
                raw = self.run_projection(
                    p, [(self.beta, p.n_days + PREFIT_ADDITIONAL_DAYS)]
                )
                self.i_day = i_day = int(
                    get_argmin_ds(
                        raw["census_hospitalized"], p.current_hospitalized
                    )
                )
                ...
                for self.i_day in range(p.n_days + PREFIT_ADDITIONAL_DAYS):
                    ...
                    loss = get_loss(
                        raw["census_hospitalized"][self.i_day],
            ...
            intrinsic_growth_rate = get_growth_rate(p.doubling_time)
            self.beta = get_beta(
                intrinsic_growth_rate, self.gamma, self.susceptible, 0.0
            )
            self.beta_t = get_beta(
                intrinsic_growth_rate,
                self.gamma,
        ...
        self.raw["date"] = self.raw["day"].astype(
            "timedelta64[D]"
        ) + np.datetime64(p.current_date)

        self.raw_df = pd.DataFrame(data=self.raw)
        self.dispositions_df = pd.DataFrame(
            data={
                "day": self.raw["day"],
                "date": self.raw["date"],
        ...
        logger.info(
            "len(np.arange(-i_day, n_days+1)): %s",
            len(np.arange(-self.i_day, p.n_days + 1)),
        )
        ...
        doubling_time_t = 1.0 / np.log2(self.beta_t * susceptible - gamma + 1)
        self.doubling_time_t = doubling_time_t

        self.sim_sir_w_date_df = build_sim_sir_w_date_df(
            self.raw_df, p.current_date, self.keys
        )

        self.sim_sir_w_date_floor_df = build_floor_df(
            self.sim_sir_w_date_df, self.keys, ""
        )
        self.admits_floor_df = build_floor_df(
            self.admits_df, p.dispositions.keys(), "admits_"
        )
        self.census_floor_df = build_floor_df(
            self.census_df, p.dispositions.keys(), "census_"
        )
    ...
    def run_projection(
        self, p: Parameters, policy: Sequence[Tuple[float, int]]
    ):
        raw = sim_sir(
            self.susceptible,
            self.infected,
...
def build_sim_sir_w_date_df(
    raw_df: pd.DataFrame, current_date: datetime, keys: Sequence[str],
) -> pd.DataFrame:
    day = raw_df.day
    return pd.DataFrame(
        {
...
def calculate_census(
    raw: Dict, lengths_of_stay: Dict[str, int],
):
