C=======================================================================
C  COPYRIGHT 1998-2018 DSSAT Foundation
...
C=======================================================================
C
C     CROPPING SYSTEM MODEL Version 4.7.2
C
C     Decision Support System for Agrotechnology Transfer (DSSAT)
C
C     May 2018  CSM Version 4.7.2
C
...
!      call path_adj(pathx)
     ...
     &    THEN
C        CALL ERROR (ERRKEY,4,DSSATPRO,0)
      ELSE
...
C           IF (ERRNUM .NE. 0) CALL ERROR (ERRKEY,28,FILEB,LINBIO)
           CALL FIND (LUNBIO,FINDCH,LINBIO,IFIND)
C           IF (IFIND .EQ. 0) CALL ERROR (ERRKEY,26,FILEB,LINBIO)
        ENDIF
      ENDIF 
...
C        CALL INFO(8,ERRKEY,MSG)
      ELSE
C        CALL INFO(7,ERRKEY,MSG)
      ENDIF

      WRITE (*,10) "CALLING LAND: 1"
 10   FORMAT (A)
...
C***********************************************************************
      ...
      IF (MULTI .GT. 1) THEN
        RUN   = RUN + 1
C        CALL MULTIRUN(RUN, 0)  !chp 3/17/2011
        YRSIM = YRSIM_SAVE
        CALL YR_DOY(YRSIM,YR,ISIM)
...
!     Final end-of-run call to land unit module
      ...
      CALL PUT(CONTROL)
      WRITE (*,10) "CALLING LAND: 7"
      CALL LAND(CONTROL, ISWITCH, 
     &          YRPLT, MDATE, YREND)

      !Change output file names if FNAME set
C      CALL OPNAMES(FNAME)
