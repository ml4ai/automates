...
C               TSRFN(I) = TA
C            ENDDO
C            SRFTEMP = TSRFN(3)    !LPM 04DEC14 to include the surface temperature as output
C            CALL OPSTEMP(CONTROL, ISWITCH, DOY, SRFTEMP, ST,TAV,TAMP)  !LPM
C          
...
C        IF (MEEVP .EQ. 'Z') THEN
C          CALL ETIND(
C     &      DUL2, RLV, SALBW, SW,                         !Input
C     &      CEN,DAYRAD,DLAYR2,DULE,DYABSR,DYINTR,EDAY,    !Output
C     &      EOP,ETNOON,FRDFRN,LLE,NELAYR,NLAYR,PCABRN,    !Output
C     &      PCINRN,RADN,RLV2, SALB, SHCAP,ST2,STCOND,SW2, !Output
C     &      SWE, TDAY,TEMPN,TSRF,TSRFN,XSW,YSCOND,YSHCAP) !Output
C
C          CALL ROOTWU(RATE,
C     &      DLAYR, LL, NLAYR, PORMIN, RLV, RWUMX, SAT, SW,!Input
C     &      RWU, TRWUP)                           !Output
C
CC  KJB NOTE TO CP.  NEED TO DELETE RWUH HERE, PUT IT INTO HOURLY
CC  KJB CALL TO ROOTWU, DO WE INITIATE HOURLY? PRIOR TO HOURLY RATE?
CC         Increase root water uptake rate 5-fold to account for
CC         instantaeous vs. daily rates and convert to mm/h.
C          !RWUH = 5.0 * TRWUP * 10.0/24.0
C        ENDIF
