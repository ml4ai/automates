...
C            IF (PATHL .EQ. 1) THEN
C                 CALL ERROR (ERRKEY,3,DSSATP,I)
C            ENDIF
...
C      IF (PFLAG .EQ. 1) THEN
C         WRITE(*,600) PROCOD, DSSATP
C 600     FORMAT(' CODE ',A3,' not found in file ',A)
C         CALL ERROR (ERRKEY,1,DSSATP,I)
C      ENDIF
...
C      ELSE
C         CALL ERROR (ERRKEY,2,DSSATP,0)
      ENDIF
...
C      IF (ISECT .EQ. 0) THEN
C         MSG(1) = "Code not found in DSSATPRO file."
C         MSG(2) = "File: " // DSSATP(1:72)
C         WRITE(MSG(3),'("Code = ",A3)') PROCOD
C         MSG(4) = "Program will end."
C         CALL WARNING(4,ERRKEY,MSG)
C         CALL ERROR (ERRKEY,3,DSSATP,LINPR)
C      ELSE
      IF (LINE(1:3) .NE. PROCOD) THEN
         GO TO 100
      ELSE
...
C      IF (ERRNUM /= 0) CALL ERROR (ERRKEY,ERRNUM,DSSATP,0)
