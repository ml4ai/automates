      subroutine ch_rtsed_Molinas_Wu
...
!!    Molinas and Wu strempower approach combined with Einsteins deposition equation
...
!!    Settling velocity for ch_d50(jrch) particle size
	w50 = 411.0 * ((ch_sed(jsed)%bed_d50/1000.)**2.) / (3600.)
      USpower = (vc ** 3.)/((2.65 - 1.) * 9.81 * rchdep * w50           &        
             * ((log10(rchdep/(ch_sed(jsed)%bed_d50 * 1e-006)))**2.))
...
!! compute changes in channel dimensions
      ...
      return
      end subroutine ch_rtsed_Molinas_Wu