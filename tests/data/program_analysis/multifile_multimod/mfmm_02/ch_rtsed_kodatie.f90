...
!!    Plus particle size tracking.
      ...
      real :: akod_a                !units         |description
      real :: akod_b                !units         |description
      real :: akod_c                !units         |description
      real :: akod_d                !units         |description
      real :: qcych                 !units         |description
...
!!    Maximum bedmaterial transport capacity in metric tons/m/day
      qcych =akod_a * (vc**akod_b) * (rchdep**akod_c) *                 &                
                                         (ch_hyd(jhyd)%s**akod_d)

!!    Maximum bedmaterial transport capacity in metric tons/day
      cych = qcych/qdin * (topw + ch_vel(jrch)%wid_btm) / 2.
...
!! compute changes in channel dimensions
      ...
      return
      end subroutine ch_rtsed_kodatie