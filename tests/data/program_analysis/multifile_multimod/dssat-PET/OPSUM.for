...
C        IF (FMOPT == 'C') THEN
C            
C            CALL CsvOutSumOpsum(RUN, TRTNUM, ROTNO, ROTOPT, CRPNO, CROP,
C     &MODEL, CONTROL%FILEX(1:8), TITLET, FLDNAM, WSTAT, SLNO, YRSIM, 
C     &YRPLT, EDAT, ADAT, MDAT, YRDOY, DWAP, CWAM, HWAM, HWAH, BWAH, 
C     &PWAM, HWUM, HNUMUM, HIAM, LAIX, HNUMAM, IRNUM, IRCM, PRCM, ETCM,
C     &EPCM, ESCM, ROCM, DRCM, SWXM, NINUMM, NICM, NFXM, NUCM, NLCM, 
C     &NIAM, CNAM, GNAM, N2OEC, PINUMM, PICM, PUPC, SPAM, KINUMM, KICM, 
C     &KUPC, SKAM, RECM, ONTAM, ONAM, OPTAM, OPAM, OCTAM, OCAM, CO2EC, 
C     &DMPPM, DMPEM, DMPTM, DMPIM, YPPM, YPEM, YPTM, YPIM, DPNAM, 
C     &DPNUM, YPNAM, YPNUM, NDCH, TMAXA, TMINA, SRADA, DAYLA, CO2A, 
C     &PRCP, ETCP, ESCP, EPCP,   
C     &vCsvlineSumOpsum, vpCsvlineSumOpsum, vlngthSumOpsum) 
C            
C            CALL LinklstSumOpsum(vCsvlineSumOpsum) 
C        END IF
...
C            csvICOUNT = ICOUNT
C            csvOLAP = OLAP
C            CALL CsvOutEvOpsum(EXPER, RUN, CG, TRTNUM, ROTNO,  CROP, 
C     &Simulated, Measured, ICOUNT,   
C     &vCsvlineEvOpsum, vpCsvlineEvOpsum, vlngthEvOpsum) 
C            
C            CALL LinklstEvOpsum(vCsvlineEvOpsum)
C         END IF
