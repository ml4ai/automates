...
!       Initialize TOTAML
     ...
     &    ALI, TOTAML)                                    !Output
        ...
        SELECT CASE(MEGHG)
        CASE("1")
C          CALL Denit_DayCent (CONTROL, ISWNIT, 
C     &    dD0, newCO2, NO3, SNO3, SOILPROP, SW,       !Input
C     &    DLTSNO3,                                    !I/O
C     &    CNOX, TNOXD, N2O_data)                      !Output
...
!       tillage depth
C      IF (TILLVALS % NTIL .GT. 0) THEN
C        TILDATE = TILLVALS % TILDATE
C        IF (YRDOY .EQ. TILDATE) THEN
C          !Call mixing routine for soil properties
C          MIXPCT = TILLVALS % TILMIX
C          TDEP = TILLVALS % TILDEP
C          CALL SoilMix (SNO3, DLTSNO3, 1, DLAYR, MIXPCT, NLAYR, TDEP)
C          CALL SoilMix (SNH4, DLTSNH4, 1, DLAYR, MIXPCT, NLAYR, TDEP)
C          CALL SoilMix (UREA, DLTUREA, 1, DLAYR, MIXPCT, NLAYR, TDEP)
C        ENDIF
C      ENDIF
