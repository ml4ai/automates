...
C        IF (YRDOY .EQ. TILLVALS % TILDATE) THEN
C          MIXPCT = TILLVALS % TILMIX
C          TDEP = TILLVALS % TILDEP
C          CALL SoilMix(SKi_Avail,DLTSKiAvail,1,DLAYR,MIXPCT,NLAYR,TDEP)
C          CALL SoilMix(SKi_Tot  ,DLTSKiTot  ,1,DLAYR,MIXPCT,NLAYR,TDEP)
C        ENDIF
C      ENDIF
