...
C        IF (ERR .NE. 0) CALL ERROR(ERRKEY, ERR, DATAX, 0)
        FOUND = FIND_IN_FILE(SECTION,LUN)
C        IF (FOUND .EQ. 0) CALL ERROR(SECTION, 42, DATAX, LNUM)
        DO WHILE (.TRUE.)    !.NOT. EOF(LUN)
          READ(LUN,'(A,T10,A)',END=200,IOSTAT=ERR) FCODE, FTEXT
          LNUM = LNUM + 1
C          IF (ERR .NE. 0) CALL ERROR(ERRKEY, ERR, DATAX, LNUM)
...
C        CALL ERROR(ERRKEY, 29, FILECDE, 0)
      ENDIF
  200 CONTINUE
      RETURN
...
!       Error handling
 5000   CONTINUE
        X = '   -99'
C        WRITE (MSG(1),'(" Error in FILEA - Measured data not used")')
C        CALL INFO(1, "READA ", MSG)
      ENDIF
...
C      IF (ERRNUM .NE. 0) CALL ERROR('READA ',2,'FILEA',0)
 1000 FORMAT(F6.0)
      IDAT = INT(RDAT)
