...
C
C          DO L = 1, NLAYR
C            !Depths of soil water (cm)
C            SW_UNMIX(L) = SW(L) * DLAYR(L)
C          ENDDO
C
C          CALL SoilMixing(DLAYR, MIXPCT, NLAYR, TDEP, SW_UNMIX, SW_MIX)
C
C          DO L = 1, NLAYR
C            !Change in SW due to tillage
C            SWDELTL(L) = SW_MIX(L) / DLAYR(L) - SW(L)
C          ENDDO
C         CALL SUMSW(NLAYR, DLAYR, SWDELTL, SWTOT)
C        ENDIF
C      ENDIF
!-----------------------------------------------------------------------
!     Save infiltration and runoff information for floodwater calcs
