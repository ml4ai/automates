...
C      IF (CROPD .EQ. '          ') THEN
C        WRITE(MSG(1),11) CROP , "DETAIL.CDE"
C   11   FORMAT('Crop code ',A2, ' could not be found in file: ',A)
...
C
C        INQUIRE (FILE="HEADER.OUT", EXIST=FEXIST)
C        IF (FEXIST) THEN
C
C!         Open temporary batch file
C          CALL GETLUN("OUTBAT", LUNTMP)
C          OPEN (LUNTMP, FILE="TEMP.BAT", STATUS = 'REPLACE')
C          WRITE(LUNTMP,'(A)') "ECHO OFF"
C          WRITE(LUNTMP,'(A)') "COPY /Y *.OUT *.BAK"
C          WRITE(LUNTMP,'(A)') "ERASE *.OUT"
C          CLOSE (LUNTMP)
C
C          SYS = SYSTEM("TEMP.BAT >TEMP.BAK")
C          
C!         Delete TEMP.BAT file
C          OPEN (LUNTMP, FILE = "TEMP.BAT", STATUS = 'UNKNOWN')
C          CLOSE (LUNTMP, STATUS = 'DELETE')
C  
C!         Delete TEMP.BAK file
C          OPEN (LUNTMP, FILE = "TEMP.BAK", STATUS = 'UNKNOWN')
C          CLOSE (LUNTMP, STATUS = 'DELETE')
C
C!         Output.CDE file is missing
C          WRITE(MSG(1),'(A,A)') "OUTPUT.CDE file not found."
C          MSG(2)=
C     &          "Previous *.OUT files will be saved as *.BAK files."
C          CALL WARNING(2,'CSM',MSG)
C        ENDIF
      ENDIF
