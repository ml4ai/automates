...
C     &      1X,F6.1,1X,F6.1,1X,F6.1,1X,F6.1,1X,F6.1,1X,F6.1)
        ...
        !     VSH     
C      IF (FMOPT == 'C') THEN 
C         CALL CsvOutETPhot(EXPNAME, CONTROL%RUN, CONTROL%TRTNUM, 
C     &CONTROL%ROTNUM, CONTROL%REPNO, YEAR, DOY, DAS, 
C     &PCINPD, PG, PGNOON, PCINPN, SLWSLN, SLWSHN, PNLSLN, 
C     &PNLSHN, LMXSLN, LMXSHN, TGRO, TGROAV,   
C     &vCsvlineETPhot, vpCsvlineETPhot, vlngthETPhot)
C     
C         CALL LinklstETPhot(vCsvlineETPhot)
C      END IF         
