...
C      IF (FMOPT == 'C') THEN
C         CALL CsvOut(EXPNAME,CONTROL%RUN, CONTROL%TRTNUM,CONTROL%ROTNUM
C     &,CONTROL%REPNO, YEAR, DOY, DAS, DAP, VSTAGE, RSTAGE, XLAI, 
C     &WTLF, STMWT, SDWT, RTWT, VWAD, TOPWT, SEEDNO, SDSIZE, HI, PODWT,
C     &PODNO, SWF_AV, TUR_AV, NST_AV, PS1_AV, PS2_AV, KST_AV, EXW_AV,
C     &PCNLP, SHELPC, HIP, PODWTD, SLAP, CANHT, CANWH,
C     &DWNOD, RTDEP, N_LYR, RLV, CUMSENSURF, CUMSENSOIL,
C     &vCsvline, vpCsvline, vlngth)
C     
C         CALL Linklst(vCsvline)
C      END IF
...
C-----------------------------------------------------------------------
  ...
  410     FORMAT(1X,I4,1X,I3.3,2(1X,I5),3(1X,F7.1),2(1X,F7.2),1X,
     ...
     &       2(1X,F7.2))
          ...
          !     VSH
C      IF (FMOPT == 'C') THEN
C         CALL CsvOutPlNCrGro(EXPNAME, CONTROL%RUN, CONTROL%TRTNUM, 
C     &CONTROL%ROTNUM, CONTROL%REPNO, YEAR, DOY, DAS, DAP, 
C     &WTNCAN, WTNSD, WTNVEG, PCNSDP, PCNVEG, WTNFX, WTNUP, 
C     &WTNLF, WTNST, PCNLP, PCNSTP, PCNSHP, PCNRTP, NFIXN, 
C     &CUMSENSURFN, CUMSENSOILN,
C     &vCsvlinePlNCrGro, vpCsvlinePlNCrGro, vlngthPlNCrGro)
C     
C         CALL LinklstPlNCrGro(vCsvlinePlNCrGro)
C      
C         CALL CsvOutPlCCrGro(EXPNAME, CONTROL%RUN, CONTROL%TRTNUM, 
C     &CONTROL%ROTNUM, CONTROL%REPNO, YEAR, DOY, DAS, DAP, 
C     &TOTWT, PG, CMINEA, GROWTH, GRWRES, MAINR, CADLF, 
C     &CADST, RHOLP, RHOSP, TGRO, TGROAV, PCNSDP, PCLSDP, 
C     &PCCSDP, TS, 
C     &vCsvlinePlCCrGro, vpCsvlinePlCCrGro, vlngthPlCCrGro)
C      
C         CALL LinklstPlCCrGro(vCsvlinePlCCrGro)    
C      END IF
C-----------------------------------------------------------------------
          IF (FMOPT == 'A' .OR. FMOPT == ' ') THEN ! VSH
