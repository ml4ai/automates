...
!     &      CumResE(N), NINT(SNDD), NINT(SOND), SomLitE(0,N), 
!     &        NINT(TSOME(N)+TLITE(N)), NINT(TSOME(N)), TLITE(N),
!     &      CumResE(P), NINT(SPDD), NINT(SOPD), SomLitE(0,P),  
!     &        NINT(TSOME(P)+TLITE(P)), NINT(TSOME(P)), TLITE(P)
...
C      IF (FMOPT == 'C') THEN 
C        IF (N_ELEMS > 1) Then
C         CALL CsvOutSoilOrg1(EXPNAME, CONTROL%RUN, CONTROL%TRTNUM, 
C     &CONTROL%ROTNUM, CONTROL%REPNO, YEAR, DOY, DAS, 
C     &CumRes, SCDD, SOCD, SomLitC, TSOMC, TLITC, CumResE, 
C     &SNDD, SOND, SomLitE, TSOME, TLITE, SPDD, SOPD,  
C     &vCsvlineSoilOrg, vpCsvlineSoilOrg, vlngthSoilOrg)
C     
C         CALL LinklstSoilOrg(vCsvlineSoilOrg)
C        ELSE
C         CALL CsvOutSoilOrg2(EXPNAME, CONTROL%RUN, CONTROL%TRTNUM, 
C     &CONTROL%ROTNUM, CONTROL%REPNO, YEAR, DOY, DAS, 
C     &CumRes, SCDD, SOCD, SomLitC, TSOMC, TLITC, CumResE, 
C     &SNDD, SOND, SomLitE, TSOME, TLITE,   
C     &vCsvlineSoilOrg, vpCsvlineSoilOrg, vlngthSoilOrg)
C     
C         CALL LinklstSoilOrg(vCsvlineSoilOrg)
C        END IF
