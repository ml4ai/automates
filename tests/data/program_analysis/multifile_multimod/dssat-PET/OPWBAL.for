...
!         New print format includes mulch, tiledrain and runoff info
 ...
 1302     FORMAT(1X,I4,1X,I3.3,3(1X,I5),3(1X,I6),3(1X,I5),
     &      10(F8.3))
      ...
      IF (FMOPT == 'C') THEN
         N_LYR = MIN(10, MAX(4,SOILPROP%NLAYR)) 
C         CALL CsvOutSW_crgro(EXPNAME,CONTROL%RUN, CONTROL%TRTNUM,
C     &CONTROL%ROTNUM,CONTROL%REPNO, YEAR, DOY, DAS, TSW, PESW, TRUNOF,
C     &TDRAIN, CRAIN, NAP, TOTIR, AVWTD, MULCHWAT, TDFD*10., TDFC*10.,
C     &RUNOFF, N_LYR, SW, vCsvlineSW, vpCsvlineSW, vlngthSW)
C     
C         CALL LinklstSW(vCsvlineSW)
      END IF     
