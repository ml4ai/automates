...
!       Initialize soil evaporation variables
        SELECT CASE (MESEV)
!     ----------------------------
C        CASE ('R')  !Original soil evaporation routine
C          CALL SOILEV(SEASINIT,
C     &      DLAYR, DUL, EOS, LL, SW, SW_AVAIL(1),         !Input
C     &      U, WINF,                                      !Input
C     &      ES)                                           !Output
!     ----------------------------
...
C!         First meet evaporative demand from floodwater
C          IF (FLOOD .GT. 1.E-4) THEN
C            CALL FLOOD_EVAP(XLAI, EO, EF)   
C            IF (EF > FLOOD) THEN
C!             Floodwater not enough to supply EOS demand
C              EOS_SOIL = MIN(EF - FLOOD, EOS)
C              EF = FLOOD
C            ELSE
C              EOS_SOIL = 0.0
C            ENDIF
