...
C      IF (CROP .NE. 'FA' .AND. MEPHO .EQ. 'C') THEN
C        CALL PHOTO(CONTROL, 
C     &    BETN, CO2, DXR57, EXCESS, KCAN, KC_SLOPE,       !Input
C     &    NR5, PAR, PStres1, SLPF, RNITP, SLAAD,          !Input
C     &    SWFAC, TDAY, XHLAI, XPOD,                       !Input
C     &    AGEFAC, PG)                                     !Output
C      ENDIF
...
C        CALL PEST(CONTROL, ISWITCH, 
C     &    AREALF, CLW, CSW, LAGSD, LNGPEG, NR2, PGAVL,    !Input
C     &    PHTIM, PLTPOP, RTWT, SLA, SLDOT, SOILPROP,      !Input
C     &    SSDOT, STMWT, TOPWT, WLFDOT, WTLF, YRPLT,       !Input
C     &    RLV, SDNO, SHELN, SWIDOT,                       !Input/Output
C     &    VSTAGE, WSHIDT, WTSD, WTSHE,                    !Input/Output
C     &    ASMDOT, DISLA, NPLTD, PPLTD,                    !Output
C     &    SDDES, WLIDOT, WRIDOT, WSIDOT,SDWT)                  !Output
C      ENDIF
...
!-----------------------------------------------------------------------
C        IF (DETACH .EQ. 'Y') THEN
C          CALL PODDET(
C     &      FILECC, TGRO, WTLF, YRDOY, YRNR2,             !Input
C     &      PODWTD, SDNO, SHELN, SWIDOT,                  !Output
C     &      WSHIDT, WTSD, WTSHE,                          !Output
C     &      RUNINIT)                                      !Control
C        ENDIF
...
!-----------------------------------------------------------------------
      ENDIF
!----------------------------------------------------------------------
C      IF (DETACH .EQ. 'Y' .AND. DAS .LE. NVEG0+1) THEN
C        CALL PODDET(
C     &    FILECC, TGRO, WTLF, YRDOY, YRNR2,               !Input
C     &    PODWTD, SDNO, SHELN, SWIDOT,                    !Output
C     &    WSHIDT, WTSD, WTSHE,                            !Output
...
C-----------------------------------------------------------------------
C      IF (ISWPHO .EQ. 'Y' .OR. ISWPHO .EQ. 'H') THEN
C        CALL P_CGRO (DYNAMIC, ISWITCH, 
C     &    CROP, FILECC, MDATE, PCNVEG, PLTPOP, RLV,       !Input
C     &    RootMob, RTDEP, RTWT, SDWT, SeedFrac,           !Input
C     &    ShelMob, SHELWT, ShutMob, SOILPROP,             !Input
C     &    SPi_AVAIL, STMWT, SWIDOT, VegFrac, WLIDOT,      !Input
C     &    WRIDOT, WSHIDT, WSIDOT, WTLF, YRPLT,            !Input
C     &    SENESCE,                                        !I/O
C     &    PConc_Shut, PConc_Root, PConc_Shel, PConc_Seed, !Output
C     &    PStres1, PStres2, PUptake, FracRts)             !Output
C      ENDIF
...
C         Pod Detachment
C-----------------------------------------------------------------------
C      IF (DETACH .EQ. 'Y' .AND. DAS .GE. NR1) THEN
C        CALL PODDET(
C     &    FILECC, TGRO, WTLF, YRDOY, YRNR2,               !Input
C     &    PODWTD, SDNO, SHELN, SWIDOT,                    !Output
C     &    WSHIDT, WTSD, WTSHE,                            !Output
...
C        CALL FREEZE(
C     &    FREEZ2, IDETO, NOUTDO, NRUSLF, SLDOT,           !Input
C     &    TMIN, WTLF, YRDOY,  YRPLT,                      !Input
C     &    MDATE,                                          !Input/Output
C     &    WLFDOT)                                         !Output
      ELSE
          WLFDOT = 0.0
      ENDIF
...
C     &      PHTIM, PLTPOP, RTWT, SLA, SLDOT, SOILPROP,      !Input
C     &      SSDOT, STMWT, TOPWT, WLFDOT, WTLF, YRPLT,       !Input
C     &      RLV, SDNO, SHELN, SWIDOT,                       !I/O
C     &      VSTAGE, WSHIDT, WTSD, WTSHE,                    !I/O
C     &      ASMDOT, DISLA, NPLTD, PPLTD,                    !Output
C     &      SDDES, WLIDOT, WRIDOT, WSIDOT,SDWT)             !Output
C        ENDIF
