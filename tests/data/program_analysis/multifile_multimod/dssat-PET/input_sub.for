...
!!!           WRITE (*,400)
         ...
         IF (NSENS .EQ. 1) THEN
            INITIAL = (ISWWAT .EQ.'N')
C            CALL SENS (NSENS,VARNO,VARTY,VRNAME,FTYPEN,LNIC,LNSA,
C     &        WRESR,WRESND,ISIM,NYRS,IPLT,WMODI,ECONO,ECONAM,ECOTYP,
C     &        PRCROP,SWINIT,INO3,INH4,RUN,FROP,YRIC,EFINOC,EFNFIX,
C     &        CROP,IVRGRP,ISENS,MODEL, RNMODE, FILEX,FILEX_P, 
C     &        ISWITCH,CONTROL)
            IF (INITIAL) THEN
