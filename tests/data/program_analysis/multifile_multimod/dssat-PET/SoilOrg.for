...
C        IF (YRDOY .EQ. TILDATE) THEN
C!         Incorporate surface residue
C          TDEP = TILLVALS % TILDEP
C          TILRESINC = TILLVALS % TILRESINC
C
C!         Amount of surface mulch to be incorporated
C          RESINC  = MULCH % MULCHMASS * TILRESINC / 100.
C          RESINCN = MULCH % MULCHN * TILRESINC / 100.
C          RESINCP = MULCH % MULCHP * TILRESINC / 100.
C
C          IF (RESINC .GT. 0.001 .AND. TDEP .GT. 0.001) THEN 
C            DLTSRF  = DLTSRF  - RESINC
C            DLTSRFN = DLTSRFN - RESINCN
C            DLTSRFP = DLTSRFP - RESINCP
C
C!           Distribute thru tillage depth
C            DEPTH = 0.0
C            DO L = 1, NLAYR
C              HOLD  = DEPTH               !Depth to top of layer
C              DEPTH = DEPTH + DLAYR(L)    !Depth to bottom of layer
C              IF (TDEP .LE. DEPTH) THEN
C!               FR = fraction of the residue added to this layer
C                FR = (TDEP - HOLD) / TDEP
C              ELSE
C                FR = DLAYR(L) / TDEP
C              ENDIF   
C
C!             Add the residue to the carbohydrate, cellulose and lignin
C!             pools of the layer.
C              ADD = RESINC * FR
C              DltFPOOL(L,1) = DltFPOOL(L,1) + ADD * PRCHO 
C              DltFPOOL(L,2) = DltFPOOL(L,2) + ADD * PRCEL 
C              DltFPOOL(L,3) = DltFPOOL(L,3) + ADD * PRLIG 
C
C!             Add the residue to the rate variables.
C              DLTFON(L) = DLTFON(L) + RESINCN * FR        !kg[N]/ha
C              DLTFOP(L) = DLTFOP(L) + RESINCP * FR        !kg[P]/ha
C
C!             If there are no more soil layers over which to distribute
C!             the residue, jump out of the DO loop. 
C              IF (DEPTH >= TDEP) EXIT
C            ENDDO   !End of loop on soil layers.
C          ENDIF   !End of IF block on RESSOL and RESDEPTH.
C
C!         Call mixing routine for soil properties
C          MIXPCT = TILLVALS % TILMIX
C
C          CALL SoilMix (SSOMC, DLTHUMC, 0, DLAYR, MIXPCT, NLAYR, TDEP)
C          CALL SoilMix (FON,   DLTFON,  1, DLAYR, MIXPCT, NLAYR, TDEP)
C          CALL SoilMix (FOP,   DLTFOP,  1, DLAYR, MIXPCT, NLAYR, TDEP)
C
C          DO IEL = 1, N_ELEMS
C            CALL SoilMix (SSOME, DLTHUME, 0,IEL,DLAYR,MIXPCT,NLAYR,TDEP)
C          ENDDO
C
C          DO I = 1, 3
C            CALL SoilMix (FPOOL, DLTFPOOL,1, I, DLAYR,MIXPCT,NLAYR,TDEP)
C          ENDDO
C        ENDIF
      ENDIF
...
!       The reduction in the SSOME pool due to its decomposition is
!       calculated from the amount of N mineralized from the humus and
!       assuming that 20% of N from FON becomes stabilized as SSOME.
...
C     Write seasonal output
     ...
     &    FON, HARVRES, IMM, MNR, MULCH, N_ELEMS, NLAYR,  !Input
