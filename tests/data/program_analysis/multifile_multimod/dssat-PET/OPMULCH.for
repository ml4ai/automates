...
!!!        INQUIRE (FILE = "Mulch.OUT", EXIST = FEXIST)
  ...
  200   FORMAT(1X,I4,1X,I3.3,1X,I5,F8.3,F8.2,I8,F8.2)
        ...
        !     VSH
C      IF (FMOPT == 'C') THEN 
C         CALL CsvOutMulch(EXPNAME, CONTROL%RUN, CONTROL%TRTNUM, 
C     &CONTROL%ROTNUM, CONTROL%REPNO, YEAR, DOY, DAS, 
C     &MULCH % MULCHCOVER, MULCH % MULCHTHICK,  
C     &MULCH % MULCHMASS, MULCH %MULCHWAT,   
C     &vCsvlineMulch, vpCsvlineMulch, vlngthMulch)
C     
C         CALL LinklstMulch(vCsvlineMulch)
C      END IF
