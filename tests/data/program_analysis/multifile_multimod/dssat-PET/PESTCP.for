...
C             Note:  PLAIW is calculated here, but not used anywhere.
	        ...
	        BETALS = PDCF1(I,J)
			  PNPDLA = PL(K)
                CALL PUT('PDLABETA','BETA',BETALS)
                CALL PUT('PDLABETA','PDLA',PNPDLA)
                PDLA = PDLA + DAM
              ENDIF
...
C-----------------------------------------------------------------------
            IF (INDEX( PCPID(I,J),'TOPWT') .GT. 0.0) THEN
              IF (TOPWT .GT. 1.E-4 .AND. TOPWT .GT. PL(K)/10.) THEN
C-----------------------------------------------------------------------
