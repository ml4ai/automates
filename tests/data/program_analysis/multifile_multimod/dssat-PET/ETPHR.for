...
C  Computes canopy ET (mm/h) and gross photosynthesis (�mol CO2/m2/s)
C  for each hour.
C-----------------------------------------------------------------------
C  REVISION HISTORY
...
C          DO WHILE (REPEAT .AND. ITER .LE. 5)
C            TCPREV = TCAN
C            CALL CANOPG(
C     &        CO2HR, FNPGL, FNPGN, LAISH, LAISL, LMXREF,  !Input
C     &        LNREF, NSLOPE, PARSH, PARSUN, QEREF, RNITP, !Input
C     &        SLAAD, SLWSLO, TMIN, TSURF, TYPPGL, TYPPGN, !Input
C     &        XLMAXT, YLMAXT,                             !Input
C     &        AGEFAC, CONDSH, CONDSL, CSHSTR, CSLSTR,     !Output
C     &        LFMXSH, LFMXSL, PCNLSH, PCNLSL, PGHR,       !Output
C     &        SLWREF, SLWSH, SLWSL, STRESS,              !Output
C     &        CCNEFF,CICAD,CMXSF,CQESF,PGPATH,           !Input
C     &        AGEQESL,CO2QESL,QEFFSL)                    !Output
C            CALL CANPET(
C     &        CANHT, CEC, CEN, CLOUDS, CONDSH, CONDSL,    !Input
C     &        DLAYR2, FRACSH, FRSHV, KDIRBL, LAISH,       !Input
C     &        LAISHV, LAISL, LAISLV, LWIDTH, RABS,        !Input
C     &        RCUTIC, REFHT, RHUMHR, STCOND, TAIRHR,      !Input
C     &        WINDHR,                                     !Input
C     &        EHR, RA, TCAN, THR, TSHR, TSURF, USTAR,     !Output
C     &        RB, RSURF, RNET,                         !Output
C     &        G, LH, LHEAT, RSSH, RSSL, RSSS, SH, SHEAT,  !Output
CC        G, LH, LHEAT, RSSH, RSSL, RSSS, SH, SHEAT
CC         RB, RSURF RNET output added DEC2014 by Bruce Kimball
C     &        RBSL, RBSL, RBSS)                           !Output
CC          added by BAK on 10DEC2015           
C
C            TSUM = TSUM + TCAN
C            IF (ITER .GT. 5) THEN
C              TCAN = TSUM / ITER
C            ELSE
C              TCAN = (TCAN+TCPREV) / 2.0
C            ENDIF
C            ERRTC = ABS(TCAN-TCPREV)
C            REPEAT = ERRTC .GT. EMAXTC
C            ITER = ITER + 1
C          ENDDO
C          T0HR = THR
C
CC         Water stress loop.  Evapotranspiration limited to by soil water
CC         supply by adjusting leaf conductances.  Photosynthesis recalulated.
C
C          IF (THR .GT. RWUH) THEN
C            CSLPRV = CONDSL
CC            CONDSL = CONDSL * (THR-RWUH)/THR
C            CONDSL = CONDSL * RWUH/THR
C            CSHPRV = CONDSH
CC            CONDSH = CONDSH * (THR-RWUH)/THR
C            CONDSH = CONDSH * RWUH/THR
C            REPEAT = .TRUE.
C            ITER = 1
C            TSUM = 0.0
C            DO WHILE (REPEAT .AND. ITER .LE. 5)
...
CC        G, LH, LHEAT, RSSH, RSSL, RSSS, SH, SHEAT
CC         RB, RSURF RNET output added DEC2014 by Bruce Kimball
C     &   RBSH, RBSL, RBSS)                               !Output
CC       preveious line added by BAK on 10DEC2015
C
C              TSUM = TSUM + TCAN
C              IF (ITER .GT. 5) THEN
C                TCAN = TSUM / ITER
...
C              ENDIF
C              ERRTC = ABS(TCAN-TCPREV)
C              ERRTR = ABS(THR-RWUH)
C              HOLD = CSLPRV+(CONDSL-CSLPRV)/(THR-TPREV)*(RWUH-TPREV)
C              CSLPRV = CONDSL
C              CONDSL = MAX(1.0/RCUTIC,HOLD)
C              HOLD = CSHPRV+(CONDSH-CSHPRV)/(THR-TPREV)*(RWUH-TPREV)
C              CSHPRV = CONDSH
C              CONDSH = MAX(1.0/RCUTIC,HOLD)
C              REPEAT = ERRTR .GT. EMAXTR .AND. ERRTC .GT. EMAXTC
C              ITER = ITER + 1
C            ENDDO
C            CSLSTR = CONDSL
C            CSHSTR = CONDSH
...
C     &        G, LH, LHEAT, RSSH, RSSL, RSSS, SH, SHEAT,  !Output
CC        G, LH, LHEAT, RSSH, RSSL, RSSS, SH, SHEAT
CC         RB, RSURF RNET output added on 1DEC2014 by Bruce Kimball
C     &        RBSL, RBSL, RBSS)                           !Output
CC             added by BAK on 10DEC2015
C
C            TSUM = TSUM + TCAN
...
C  Computes instantaneous canopy photosynthesis (�mol CO2/m2/s) and leaf
...
C     Compute canopy photosynthesis (�mol CO2/m2/s).
...
CC         RB, RSURF RNET output added DEC2014 by Bruce Kimball
C     &    RBSH, RBSL, RBSS)                               !Output
CC       added by BAK on 10DEC15
C
C!     ------------------------------------------------------------------
C      USE ModuleDefs     !Definitions of constructed variable types, 
C                         ! which contain control information, soil
C                         ! parameters, hourly weather data.
C      IMPLICIT NONE
C      SAVE
C
C      INTEGER I
C      REAL CANHT,CONDSH,CONDSL,CEC,CEN,DLAYR1,DLAYR2(NL),
C     &  EAIRHR,ECAN,ESATHR,EHR,ETHR,FRACSH,FRSHV,G,KDIRBL,LAISH,
C     &  LAISL,LH,LHEAT(3,1),LHVAP,LWIDTH,PATM,PSYCON,RA,REFHT,RHUMHR,
C     &  RCUTIC,RL(3,3),RABS(3),RNTOT,RNET(3,1),RS(3,3),SHAIR,STCND1,
C     &  STCOND(NL),TAIRHR,TCAN,THR,TSHR1,TSHR(NL),TSURF(3,1),VHCAIR,
C     &  VPD(3,1),VPSAT,WINDHR,CLOUDS,DAIR,DAIRD,DVAPOR,Q,SH,SHEAT(3,1),
C     &  SHAIRD,TK,MWATER,RGAS,MAIR,LAISHV,LAISLV,RADBK(3),
C     &  USTAR,XLAI,ZERO
C
C      REAL RB(3), RSURF(3),RSSH,RSSL,RSSS,RBSH,RBSL,RBSS
CC         RB, RSURF RSSH RSSL RSSS added DEC2014 by Bruce Kimball
CC         RBSH,RBSL,RBSS added by BAK on 10DEC15
C
C      PARAMETER (RGAS=8.314,MWATER=0.01802,MAIR=0.02897,PATM=101300.0,
C     &  SHAIRD=1005.0, ZERO=1.0E-6)
C
CC     Initialize.
C
C      XLAI = LAISH + LAISL
C      STCND1 = STCOND(1)
C      TSHR1 = TSHR(1)
C      DLAYR1 = DLAYR2(1) / 100.0
C
CC     Calculate air/water properties as a function of temperature.
C
C      ESATHR = VPSAT(TAIRHR)                              ! Pa
C      EAIRHR = ESATHR * RHUMHR / 100.0                    ! Pa
C      TK = TAIRHR + 273.0                                 ! K
C      DVAPOR = MWATER * EAIRHR / (RGAS * TK)              ! kg/m3
C      DAIRD = MAIR * (PATM-EAIRHR) / (RGAS * TK)          ! kg/m3
C      DAIR = DVAPOR + DAIRD                               ! kg/m3
C      Q = DVAPOR / DAIR
C      SHAIR = SHAIRD * (1.0+0.84*Q)                       ! J/kg/K
C      VHCAIR = DAIR * SHAIR                               ! J/m3/K
C      LHVAP = (2501.0-2.373*TAIRHR) * 1000.0              ! J/kg
C      PSYCON = SHAIR * PATM / (0.622*LHVAP)               ! Pa/K
C
CC     Create vpd and resistance matrices.
C
C      DO I=1,3
C        VPD(I,1) = ESATHR - EAIRHR
C      ENDDO
C      CALL ETRES(
C     &  CANHT, CEC, CEN, CONDSH, CONDSL, FRACSH, FRSHV,   !Input
C     &  KDIRBL, LAISH, LAISL, LWIDTH, RCUTIC, REFHT,      !Input
C     &  TAIRHR, TCAN, WINDHR,                             !Input
C     &  RA, RL, RS, USTAR,                                !Output
C     &  RB,RSURF)
CC          RB and RSURF Added by BAK on 1DEC2014
C        RBSL = RB(1)
C        RBSH = RB(2)
C        RBSS = RB(3)
C        IF(RBSL .GT. 20000.) THEN
C            RBSL = 20000.
C            ENDIF
C        IF(RBSH .GT. 20000.) THEN
C            RBSH = 20000.
C            ENDIF
C        IF(RBSS .GT. 20000.) THEN
C            RBSS = 20000.
C            ENDIF
CC       Obtain the resistances of sunlit, shaded leaves and
CC         soil surface. Added by BAK on 18MAR15
C
CC     Calculate NET total by subtracting net (back) longwave radiation.
C
C      CALL RADB(
C     &  CLOUDS, EAIRHR, FRSHV, LAISHV,                    !Input
C     &  LAISLV, TAIRHR, TCAN,                             !Input 
C     &  RADBK)                                            !Output
C      RNET(1,1) = RABS(1) - RADBK(1)
C      RNET(2,1) = RABS(2) - RADBK(2)
C      RNET(3,1) = RABS(3) - RADBK(3)
C      RNTOT = RNET(1,1) + RNET(2,1) + RNET(3,1)
C
CC     Solve 3-zone model for ET and E (mm/h).
C
C      CALL ETSOLV(
C     &  DLAYR1, EAIRHR, PSYCON, RL, RNET, RS, STCND1,     !Input
C     &  TAIRHR, TSHR1, VHCAIR, VPD,                       !Input
C     &  ECAN, G, LH, LHEAT, SH, SHEAT, TCAN, TSURF)       !Output
C      ETHR = LH / LHVAP * 3600.0
C      EHR = LHEAT(3,1) / LHVAP * 3600.0
C      IF (XLAI .LE. ZERO) THEN
C        TSURF(1,1) = 0.0
C        TSURF(2,1) = 0.0
C        THR = 0.0
C        EHR = ETHR
...
C      SAVE 
C
C      REAL STCND1,DLAYR1,DZ1,EAIRHR,ECAN,G,PSYCON,RBLCN,SH,LH,
C     &  TAIRHR,TCAN,TSHR1,VHCAIR,VP,VPSLOP,VSP,HOLD
C      REAL CMAT(3,1),IRL(3,3),IRS(3,3),LMAT(1,1),LHEAT(3,1),ONE(1,3),
C     &  RL(3,3),RNET(3,1),RS(3,3),RSL(3,3),RSLRN(3,1),SMAT(1,1),
C     &  SHEAT(3,1),TDIFF(3,1),TSURF(3,1),VHAIRS(3,3),VPD(3,1),
C     &  VPIRLD(3,1),VSPIRL(3,3),XMAT(3,3),YMAT(3,1)
C      DATA ONE/1.0,1.0,1.0/
C
CC     Initialize
C
C      VSP = VHCAIR * VPSLOP(TAIRHR) / PSYCON
C      VP = VHCAIR / PSYCON
C      DZ1 = DLAYR1 / 2.0
C
CC     Invert latent and sensible heat matrices to get [iRL] and [iRS].
CC     Create temporary [XMAT] = [iRSL] = [C4] then adjust for G estimated from
CC     last E-zone temperature.  [XMAT] inverted to get [RSL].
CC     VHAIRS and VSPIRL saved for later use.
C
C      CALL GAUSSJ(
C     &  RL,3,                                             !Input
C     &  IRL)                                              !Output
...
C     &  VSPIRL)                                           !Output
C      CALL MATCON(
C     &  VHCAIR,IRS,3,3,'*',                               !Input
C     &  VHAIRS)                                           !Output
C      CALL MATADD(
C     &  VHAIRS,VSPIRL,3,3,'+',                            !Input
C     &  XMAT)                                             !Output
C      XMAT(3,3) = XMAT(3,3) + STCND1/DZ1
C      CALL GAUSSJ(
C     &  XMAT,3,                                           !Input
C     &  RSL)                                              !Output
C
CC     Create [CMAT] = VP*[RSL]*[IRL]*[VPD].  Adjust [VPiRLD] = [C3] for G
CC     estimated from last E-zone temp.  Original matrix [VPIRLD] restored
CC     for later use.
C
C      CALL MATPRO(IRL,VPD,3,3,1,YMAT)
C      CALL MATCON(VP,YMAT,3,1,'*',VPIRLD)
C      HOLD = VPIRLD(3,1)
C      VPIRLD(3,1) = HOLD - STCND1/DZ1*(TSHR1-TAIRHR)
C      CALL MATPRO(RSL,VPIRLD,3,3,1,CMAT)
C      VPIRLD(3,1) = HOLD
C
CC     Solve for TDIFF matrix.
C
C      CALL MATPRO(RSL,RNET,3,3,1,RSLRN)
C      CALL MATADD(VHAIRS,VSPIRL,3,3,'+',XMAT) ! Check this--XMAT used?
C      CALL MATADD(RSLRN,CMAT,3,1,'-',TDIFF)
C      CALL MATCON(TAIRHR,TDIFF,3,1,'+',TSURF)
C
CC     Solve remaining 6 equations for latent and sensible heat fluxes and
CC     sum up latent and sensible heats for 3 zones.
C
C      CALL MATPRO(VHAIRS,TDIFF,3,3,1,SHEAT)
C      CALL MATPRO(VSPIRL,TDIFF,3,3,1,YMAT)
C      CALL MATADD(YMAT,VPIRLD,3,1,'+',LHEAT)
C      CALL MATPRO(ONE,LHEAT,1,3,1,LMAT)
C      CALL MATPRO(ONE,SHEAT,1,3,1,SMAT)
C      LH = LMAT(1,1)
C      SH = SMAT(1,1)
C
CC     Calculate other values.  RBLCN is any off-diagonal element of RS.
C
C      RBLCN = RS(1,2)
C      ECAN = (LH*RBLCN/VP) + EAIRHR
C      TCAN = (SH*RBLCN/VHCAIR) + TAIRHR
C      G = STCND1 / DZ1 * (TSURF(3,1)-TSHR1)
C
C      RETURN
C      END SUBROUTINE ETSOLV
