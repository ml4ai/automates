...
C      INTERFACE 
C        SUBROUTINE RootSoilVol(DYNAMIC, ISWPHO,  
C     &    DLAYR, DS, NLAYR,           !Input from all routines
C     &    PLTPOP, RLV, RTDEP, FILECC, !Input from plant routine
C     &    FracRts,                    !Output
C     &    LAYER, AppType)      !Input from soil module (for banded fert)
C          USE ModuleDefs
C          IMPLICIT NONE
C          CHARACTER*1,         INTENT(IN)           :: ISWPHO
C          INTEGER,             INTENT(IN)           :: DYNAMIC, NLAYR
C          REAL, DIMENSION(NL), INTENT(IN)           :: DS, DLAYR
C          REAL, DIMENSION(NL), INTENT(OUT)          :: FracRts
C          REAL,                INTENT(IN), OPTIONAL :: PLTPOP, RTDEP
C          REAL, DIMENSION(NL), INTENT(IN), OPTIONAL :: RLV
C          CHARACTER*92,        INTENT(IN), OPTIONAL :: FILECC
C          INTEGER,             INTENT(IN), OPTIONAL :: LAYER
C          CHARACTER*7,         INTENT(IN), OPTIONAL :: AppType
C        END SUBROUTINE RootSoilVol
C      END INTERFACE
!-----------------------------------------------------------------------
...
C        CALL SoilPi_init (CONTROL,       
C     &    ISWPHO, SOILPROP,                               !Input
C     &    K_ACT2LAB, K_ACT2STA, K_LAB2ACT, K_STA2ACT,     !Output
C     &    PiActive, PiLabile, PiStable, YREND)            !Output
C
C!       Error in initialization
C        IF (YREND == YRDOY) RETURN
C
C        DO L = 1, NLAYR
C!         03/01/2007 US/CHP added fraction of P in solution
C          SELECT CASE(SoilLayerType(L))
C            CASE('CALCAREOUS       '); FracPSoln(L) = 0.05
C            CASE('HIGHLYWEATHERED  '); FracPSoln(L) = 0.015
C            CASE('ANDISOL          '); FracPSoln(L) = 0.008
C            CASE DEFAULT             ; FracPSoln(L) = 0.02
C          END SELECT
C
C          SPi_Active(L) = PiActive(L) / KG2PPM(L)
C          SPi_Stable(L) = PiStable(L) / KG2PPM(L)
C          SPi_Labile(L) = PiLabile(L) / KG2PPM(L)
C
C
C          IF (NBUND .GT. 0 .OR. FLOOD .GT. 0) THEN
C             TexFac = 3.85
C	    ELSE
C	       TexFac = 0.25 + 0.25 * (SOILPROP % SAT(L)/ 
C     &                SOILPROP % LL(L))
C          ENDIF
C          WatFac(L) = AMAX1(SW(L) / SOILPROP % LL(L), 0.85)
C
C          SPi_Soluble(L)= SPi_Labile(L)* AMIN1 (1.0, 
C     &                    FracPSoln(L) * TexFac(L) * WatFac(L))
C        ENDDO
      ENDIF
...
C                CALL RootSoilVol (DYNAMIC, ISWPHO,   
C     &              DLAYR, DS, NLAYR,                           !Input 
C     &              LAYER=L, AppType=FertData%AppType,          !Input 
C     &              FracRts=FracRts)                            !Output
C
C!               Initialize root zone and non-root zone pools
C                CALL P_ROOTS_INIT(
C     &           DLAYR, FracPSoln, FracRts, KG2PPM, NLAYR,       !Input
C     &           SPi_Active, SPi_Labile, SPi_Stable, SW,         !Input
C     &           FracNoRts, PiActNoRts, PiActRts, PiLabNoRts,    !Output
C     &           PiLabRts, PiSolNoRts, PiSolRts, PiStaNoRts,     !Output
C     &           PiStaRts, PPM2KG_Rts, PPM2KG_NoRts, SPi_Soluble,!Output
C     &           SPiActNoRts, SPiActRts, SPiLabNoRts, SPiLabRts, !Output
C     &           SPiSolNoRts, SPiSolRts, SPiStaNoRts, SPiStaRts) !Output
C                FIRST = .FALSE.
              ENDIF
...
C          CALL SoilMix (SPiActNoRts, DLTSPiActNoRts, 1, DLAYR, MIXPCT, 
C     &                                                      NLAYR, TDEP)
C          CALL SoilMix (SPiLabNoRts, DLTSPiLabNoRts, 1, DLAYR, MIXPCT, 
C     &                                                      NLAYR, TDEP)
C          CALL SoilMix (SPiStaNoRts, DLTSPiStaNoRts, 1, DLAYR, MIXPCT, 
C     &                                                      NLAYR, TDEP)
        ENDIF
      ENDIF
...
C        CALL OpSOILPi(CONTROL, ISWITCH, 
C     &  CImmobP, CMinerP, FertData, PUptake,   
C     &  SOILPROP, SPi_AVAIL, SPiAvlProf, PiLabile,
C     &  SPiSolProf, SPiLabProf, SPiActProf, SPiStaProf, SPiTotProf)
C
C        CALL SoilPiBal (CONTROL, ISWITCH,
C     &    FertData, IMM, MNR, NLAYR, PUptake, 
C     &    SPiActProf, SPiLabProf, SPiStaProf)
C      ENDIF
