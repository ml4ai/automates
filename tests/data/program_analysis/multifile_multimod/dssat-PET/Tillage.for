...
C          CALL WARNING(3, ERRKEY, MSG)
C          CALL ERROR(ERRKEY,29,FILETL,0)
        ENDIF

!       Open tillage operation file.
        CALL GETLUN('FILETL', LUNTIL)
        OPEN (UNIT=LUNTIL, FILE=TILFILE, STATUS='OLD', IOSTAT=ERRNUM)
C        IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILETL,0)
...
C              IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILETL,LNUM)
