...
The following document describes the DBN-JSON schema, specifying the
JSON format that is to be generated by program analysis and consumed by
Delphi.

Here we adopt a simplified [Backus-Naur Form
(BNF)](https://en.wikipedia.org/wiki/Backus%E2%80%93Naur_form) grammar
convention combined with a convention for intuitively defining specific
JSON attribute-value lists. The schema definitions and instance DBN-JSON
examples are shown in `monospaced font`, and interspersed with
comments/discussion.
