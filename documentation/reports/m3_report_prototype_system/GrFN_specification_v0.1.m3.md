...
but follows the conventions of [javascript identifier specification rules](https://docs.javascript.org/3/reference/lexical_analysis.html#identifiers) (which includes Fortran naming syntax).
...
The optional `<code_given_name>` is used when the function identified by program analysis has also been given a name within source code. For example, in this javascript example:
...
As noted above, due to the more semantically rich identifier specification and `<identifier_string>` representation, it is not straightforward to use the `<identifier_string>` as the javascript symbol in the translated Python generated by program analysis. Instead, function and variable identifiers will be represented in the generated Python using their gensym. For debugging and visualization purposes, the generated Python code may be displayed with `<identifier_string>` (or some version that is closer to legal Python naming, although in general it does not appear to be possible to create \"safe\" Python names directly from `<identifier_string>`s).
