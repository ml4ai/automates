...
import org.clulab.grounding.{SVOGrounder, SVOGrounding, SeqOfGroundings, sparqlResult, sparqlWikiResult}
...
object ExtractAndAlign {
  ...
  def groundMentions(
    ...
    if (groundToWikidata) {
      val groundings = SVOGrounder.groundHypothesesToWiki(hypotheses, maxSVOgroundingsPerVar)
    }
    ...
    if (groundToSVO) {
      if (SVOgroundings.isDefined) {
        ...
        if (groundings.isDefined) {
          hypotheses = hypotheses ++ mkWikiLinkHypotheses(groundings.get)
        }
    ...
    if (textDefinitionMentions.isDefined) {
      linkElements(TEXT_VAR) = textDefinitionMentions.get.map { mention =>
        ...
        mkTextLinkElement(
          ...
          sentenceString = mention.sentenceObj.words.mkString(" ") //todo: filter non-content words
        )
      }
    }
...
//    println(s"grounding objects: ${groundingObjects.mkString(" || ")}")
  ...
  def mkWikiLinkHypotheses(groundings: Map[String, (Seq[sparqlWikiResult], Value)]): Seq[Obj] = {
...
//        contentType = "null"
//      )
      dstLinkElement = GrFNParser.mkWikidataElement(gr)

    } yield mkHypothesis(groundings(v)._2.obj, dstLinkElement, gr.score.get)
