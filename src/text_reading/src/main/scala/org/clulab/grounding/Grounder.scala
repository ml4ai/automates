...
//      }
  ...
  def getTerms(str: String): Seq[String] = {
    ...
    //todo have a none option
    //if no res from compounds, get each word individually
    //if headword len 1 don't do anything with it
    //make sure the output of groundmenttosvo outputs the format that will work in align
    val terms = new ArrayBuffer[String]()
      ...
      println("len outgoing: " + outgoing.length)
        ...
        for (i <- 1 to 2) {
          ...
          if (outgoingNodes.contains(idxToCheck)) {
            if (outgoing.exists(item => (item._1 == idxToCheck && item._2 == "compound" || item._1 == idxToCheck && item._2 == "amod"))) {
              println("TRUE")
...
//
//      println(indicesOfCompoundToken.mkString(" "))
//      for (idx <- indicesOfCompoundToken) {
//        val compoundWord = mention.sentenceObj.words.filter(ch => ch.length > 1).slice(idx, mention.synHead.get + 1).mkString("_")
//
////        val semHead = mention.semHeadLemma
////        compoundWords.append(compoundWord.replace(" ", "_"))
//        if (compoundWord.length > 1) {
//          compoundWords.append(compoundWord.replace("_", "-")) //todo: in the ontology, some terms are linked with a dash and some with underscore; should look for both, but find a better place for this (?)
//
//        }
//
//      }
