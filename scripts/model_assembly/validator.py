...
def validate(gromet, debug_mode=False):
    ...
    for k in SYNTAX_TYPES:
        ...
        if gromet[k] is not None:
            objects[k] = {
                **objects[k],
                **{obj["uid"]: obj for obj in gromet[k]},
            }
    ...
    objects["called_boxes"] = (
        objects["called_by_boxcall"] | objects["called_by_expr"]
    )
        ...
        for elm in objects[k]:
            ...
            if box["metadata"] is not None:
                ...
                for datum in box["metadata"]:
                    metadata_type = f"metadata_{k}_{datum['type']}"
    ...
    if gromet["metadata"] is not None:
        for datum in gromet["metadata"]:
            metadata_type = f"metadata_gromet_{datum['type']}"
            if metadata_type not in objects:
                objects[metadata_type] = [datum]
            else:
                objects[metadata_type].append(datum)
    ...
    if not (len(uid_counts.values()) == sum(uid_counts.values())):
    ...
    for v_id, variable in objects["variables"].items():
        if set(variable["states"]) <= (
            ...
            node_set = set(objects["nodes"].keys()) | set(
                objects["wires"].keys()
            )
            for s in variable["states"]:
                if s not in node_set:
                    print(
                        f"Error: Variable '{v_id}'"
    ...
    for p_id, port in objects["ports"].items():
        if port["box"] not in objects["boxes"]:
            print(
                f"Error: Parent Box of Port '{p_id}'"
        ...
        for i, (cond_id, body_id) in enumerate(conditional_box["branches"]):
            if cond_id is None:
                if i < num_branches - 1:
                    ...
                    print(
                        f"Error [Cond 1a]: condition of branch {i} in Conditional '{c_id}'"
                ...
                if cond_box["syntax"] != "Predicate":
                    ...
                    print(
                        f"Error [Cond 1b]: condition '{cond_id}' in branch {i} of Conditional '{c_id}'"
                ...
                if len(output_ports) != 1:
                    ...
                    print(
                        f"Error [Cond 3a]: condition '{cond_id}' in branch {i} of Conditional '{c_id}'"
                    ...
                    if output_port_id in objects["ports"][output_port_id]:
                        ...
                        if output_port["syntax"] != "Port":
                            ...
                            print(
                                f"Error [Cond 3b]: condition '{cond_id}' in branch {i} of Conditional '{c_id}'"
                        ...
                        if output_port["value_type"] != "Boolean":
                            ...
                            print(
                                f"Error [Cond 3c]: condition '{cond_id}' in branch {i} of Conditional '{c_id}'"
            ...
            if body_id in objects["boxes"]:
                ...
                if (
                    body_box["syntax"] != "Expression"
                    and body_box["syntax"] != "Function"
                ):
                    # (4) branch body must be Expression or Function
                    print(
                        f"Error [Cond 4]: condition '{body_id}' in branch {i} of Conditional '{c_id}'"
                ...
                for p_id in body_box["ports"]:
                    ...
                    if body_port["syntax"] != "PortCall":
                        ...
                        print(
                            f"Error [Cond 5]: condition '{body_id}' in branch {i} of Conditional '{c_id}'"
                        ...
                        if body_port["type"] == "PortInput":
                            if (
                                ...
                                print(
                                    f"Error [Cond 6a]: condition '{body_id}' in branch {i}"
                                ...
                                if called_port["type"] != "PortInput":
                                    ...
                                    print(
                                        f"Error [Cond 6b]: condition '{body_id}' in branch {i}"
                                        f" of Conditional '{c_id}'"
                            ...
                            if (
                                ...
                                print(
                                    f"Error [Cond 7a]: condition '{body_id}' in branch {i}"
                                    f" of Conditional '{c_id}'"
                                ...
                                if called_port["type"] != "PortOutput":
                                    ...
                                    print(
                                        f"Error [Cond 7b]: condition '{body_id}' in branch {i}"
                                        f" of Conditional '{c_id}'"
                                        f" has port '{p_id}' that cals port '{body_port['call']}'"
                                        f" that is type '{called_port['type']}', but it must be type PortOutput."
                                    )
                                called_ports_called_by_branch_body[
                                    called_port["uid"]
                                ].add(body_id)
                        else:
                            print(
                                f"Error [Cond x]: condition '{body_id}' in branch {i}"
                                f" of Conditional '{c_id}'"
                                f" has port '{p_id}' that cals port '{body_port['call']}'"
    ...
    for l_id in objects["loop_boxes"]:
        ...
        loop_wires_srcs = [
            wire["src"] for wire in loop_wires if wire is not None
        ]
        loop_wires_tgts = [
            wire["tgt"] for wire in loop_wires if wire is not None
        ]
        ...
        if loop_box["exit_condition"] is not None:
            ...
            if exit_condition["syntax"] != "Predicate":
                ...
                print(
                    f"ERROR [Loop 1a]: exit_condition of Loop '{l_id}' is"
            ...
            for p_id in exit_condition["ports"]:
                ...
                if port["type"] == "PortInput":
                    if p_id not in loop_wires_tgts:
                        ...
                        print(
                            f"Error [loop 1c]: Port '{p_id}' of exit_condition of Loop '{l_id}'"
                    ...
                    print(
                        f"ERROR [Loop 1d]: {port['syntax']} '{p_id}'"
            ...
            if len(exit_port) != 1:
                ...
                print(
                    f"Error [Loop 1b.1]: exit_condition '{exit_condition['uid']}' of Loop '{l_id}' "
                ...
                if exit_port["value_type"] != "Boolean":
                    ...
                    print(
                        f"Error [Loop 1b.2]: PortOutput '{exit_port['uid']}'"
                ...
                if exit_port["uid"] in loop_wires_srcs:
                    ...
                    print(
                        f"Error [Loop 1b.3]: PortOutput '{exit_port['uid']}'"
                        f" of exit_condition '{exit_condition['uid']}' of Loop '{l_id}' "
        ...
        for p_id in loop_box["ports"]:
            ...
            if port["type"] == "PortInput":
                ...
                if port["syntax"] != "Port":
                    ...
                    print(
                        f"Error [Loop 2]: PortInput '{p_id}' of Loop '{l_id}'"
        ...
        for in_p_id in input_port_ids:
            if in_p_id not in called_ports:
                # (5) Every PortInput is called by a PortOutput PortCall
                print(
                    f"Error [Loop 5]: PortInput '{in_p_id}' of Loop '{l_id}'"
    ...
    if gromet["type"] == "Bilayer":
        ...
        for w_id, wire in objects["wires"].items():
            ...
            if wire["type"] == "W_in" and src_junction and tgt_junction:
                ...
                if (
                    src_junction["type"] == "State"
                    and tgt_junction["type"] == "Flux"
                ):
                    pass  # OK
                else:
                    print(
                ...
                if (
                    src_junction["type"] == "Flux"
                    and tgt_junction["type"] == "Tangent"
                ):
                    pass  # OK
                else:
                    print(
    ...
    if gromet["type"] == "FunctionNetwork":
        ...
        if gromet["root"] is None:
            print(
                f"Error [FN root 0a]: root Box not specified ('{gromet['root']}')"
            )
        if gromet["root"] not in objects["boxes"]:
            ...
            for p_id in root_box["ports"]:
                if p_id not in objects["ports"]:
                    print(
                        f"Error [FN root 0c]: root Box '{root_box['uid']}' references"
                    ...
                    if (
                        port["type"] == "PortInput"
                        and p_id not in objects["wires_by_src_id"]
                    ):
                        print(
                            f"Error [FN root 1a]: root Box '{root_box['uid']}' has"
                            f" PortInput type Port '{p_id}'"
                            f" that does not have outgoing Wire."
                        )
                    elif (
                        port["type"] == "PortOutput"
                        and p_id not in objects["wires_by_tgt_id"]
                    ):
                        print(
                            f"Error [FN root 1b]: root Box '{root_box['uid']}' had"
                ...
                if p_id in objects["ports"]:
                    ...
                    if port_syntax == "Port" or (
                        ...
                        if p_id not in objects["wires_by_tgt_id"]:
                            print(
                                f"Error [FN 1a]: Box '{b_id}' has {port_syntax} '{p_id}'"
                                f" of type '{port_type}' with no incoming Wire."
                            )
                        elif len(objects["wires_by_tgt_id"][p_id]) != 1:
                            print(
                                f"Error [FN 1b]: Box '{b_id}' has {port_syntax} '{p_id}'"
                    ...
                    if port_syntax == "Port" or (
                        ...
                        if p_id not in objects["wires_by_src_id"]:
                            print(
                                f"Error [FN 2]: Box '{b_id}' has {port_syntax} '{p_id}'"
                    ...
                    if port_syntax == "PortCall" and port_type == "PortInput":
                        ...
                        if p_id in outgoing_wires:
                            print(
                                f"Error [FN 3]: Box '{b_id}' has {port_syntax} '{p_id}'"
                    ...
                    if port_syntax == "PortCall" and port_type == "PortOutput":
                        ...
                        if p_id in incoming_wire_ids:
                            print(
                                f"Error [FN 4]: Box '{b_id}' has {port_syntax} '{p_id}'"
                                f" of type '{port_type}' with some outgoing Wire:"
                                f" {incoming_wire_ids[p_id]}."
                            )

        # For non-Expression called Box (`non_exp_called_boxes`)
        non_exp_called_boxes = (
            objects["called_boxes"] - objects["expression_boxes"]
        )
        for b_id in non_exp_called_boxes:
            ...
            for p_id in box["ports"]:
                ...
                if port_syntax != "Port":
                    print(
                        f"Error [FN 5]: called Box '{b_id}' has a '{port_syntax}'"
                ...
                if port_type == "PortInput":
                    if p_id in objects["wires_by_tgt_id"]:
                        print(
                            f"Error [FN 6a]: called Box '{b_id}' has '{port_syntax}'"
                    ...
                    if p_id not in objects["wires_by_src_id"]:
                        print(
                            f"Error [FN 6b]: called Box '{b_id}' has '{port_syntax}'"
                ...
                if port_type == "PortOutput":
                    if p_id not in objects["wires_by_tgt_id"]:
                        print(
                            f"Error [FN 7a]: called Box '{b_id}' has '{port_syntax}'"
                    ...
                    if len(incoming_wire_ids) != 1:
                        print(
                            f"Error [FN 7b]: called Box '{b_id}' has '{port_syntax}'"
                            f" of type '{port_type}' with too many incoming Wires:"
                            f" {incoming_wire_ids}"
                            f" -- must have at just one."
                        )
        ...
        # For non-called Expressions (`non_called_expressions`)
        non_called_expressions = (
            objects["expression_boxes"] - objects["called_boxes"]
        )
        ...
        # For called Expressions (`called_expressions`)
        called_expressions = (
            objects["expression_boxes"] & objects["called_by_boxcall"]
        )
        # (11) PortInput and PortOutput should not have any incoming or outgoing Wires.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--path",
        type=str,
        default="examples/cond_ex1_gromet_FunctionNetwork.json",
