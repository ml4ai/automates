...
logging.basicConfig(filename = os.path.join(logFile_dst, f'{begin_month}-{end_month}_etree_SLE.log'),
                    level = logging.DEBUG,
                    format = Log_Format,
                    filemode = 'w')
...
def main():
    for DIR in args.directories:
        print(DIR)
        Simp_Mathml_path = f'{args.source}/{str(args.year)}/{DIR}/SLE/SLE_Simplified_mml'
