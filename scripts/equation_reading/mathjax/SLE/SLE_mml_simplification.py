...
def main():
    ...
    for DIR in args.directories:
        ...
        SLE_MathJax_MML_path = os.path.join(DIR_path, 'SLE/SLE_Mathjax_mml')

        # Making new directory for Simplified MML
        SLE_Simp_MML_path = os.path.join(DIR_path, 'SLE/SLE_Simplified_mml')
        if not os.path.exists(SLE_Simp_MML_path):
            subprocess.call(['mkdir', SLE_Simp_MML_path])

        # Creating array fro pooling
        temp = []

        for folder in os.listdir(SLE_MathJax_MML_path):
            if folder not in os.listdir(SLE_Simp_MML_path):
                temp.append([os.path.join(SLE_MathJax_MML_path, folder), folder, SLE_Simp_MML_path])
...
def Simplification(pooling_list):
    ...
    (folder_path, folder, SLE_Simp_MML_path) = pooling_list

    # Making directory named <folder> in Simp_MML_path
    SLE_Simp_MML_folder_path = os.path.join(SLE_Simp_MML_path, folder)
    if not os.path.exists(SLE_Simp_MML_folder_path):
        subprocess.call(['mkdir', SLE_Simp_MML_folder_path])
    
    #print(SLE_Simp_MML_folder_path)

    for type_of_folder in os.listdir(folder_path):
        type_of_folder_path = os.path.join(folder_path, type_of_folder)
        tyf_path = os.path.join(SLE_Simp_MML_folder_path, type_of_folder)
