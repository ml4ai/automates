...
class PassThroughFilter extends DocumentFilter {
  def filter(doc: Document): Document = doc
}

object PassThroughFilter {
  def apply(): PassThroughFilter = new PassThroughFilter
}
