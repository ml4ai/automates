...
class SensitivityModel(object):
    ...
    def sensitivity(self, N):
        ...
        if self.model == 'PETASCE':
            ...
            tG = GrFN.from_fortran_file(f"../tests/data/program_analysis/{self.model}_simple.for")
        else:
            # tG = GrFN.from_fortran_file(f"../../tests/data/program_analysis/{self.model}.for", save_file=True)
            tG = GrFN.from_fortran_file(f"../tests/data/program_analysis/{self.model}.for")

        if self.method == 'Sobol':
            (sobol_dict, timing_data) = SensitivityAnalyzer.Si_from_Sobol(N, tG, self.B, save_time= True)
        elif self.method == 'FAST':
            (sobol_dict, timing_data) = SensitivityAnalyzer.Si_from_FAST(N, tG, self.B, save_time= True)
        elif self.method == 'RBD FAST':
            (sobol_dict, timing_data) = SensitivityAnalyzer.Si_from_RBD_FAST(N, tG, self.B, save_time= True)
        else:
    ...
    def generate_dataframe(self):
        ...
        df_S1 = pd.DataFrame.from_dict(S1_max, orient='index', columns = [self.model])
        df_ST = pd.DataFrame.from_dict(ST_max, orient='index', columns = [self.model])
    ...
    def generate_indices(self):
        ...
        for i in range(len(self.sample_list)):
            ...
            S2_dataframe = pd.DataFrame(data=Si['O2_indices'], columns=Si['parameter_list'])

            sobol_dict = {'sample size':self.sample_list[i], 'S1': S1_dict, 'S2':
